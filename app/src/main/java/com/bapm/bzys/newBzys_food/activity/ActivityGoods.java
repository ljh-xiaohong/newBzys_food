...
import com.bapm.bzys.newBzys_food.adapter.GoodsAdapter;
				...
				if (result.length()>0) {
					...
					for (int i = 0; i < result.length(); i++) {
						...
						for (int j = 0; j < goodsArray.length(); j++) {
							...
							String sale_no = goodsJson.optString("SaleCount");
...
//		 CustomToast.showToast(this, msg);
		loadDialog.dismiss();
		failUtils=new LoginFailUtils(requestCode,ActivityGoods.this ,manager,ActivityGoods.this);
		failUtils.onFaile();
	}
