...
public class GoodsTypeAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.layout_delect = (LinearLayout) convertView.findViewById(R.id.layout_delect);
		...
		if (position==0){
			((ZrcListView)parent).setItemRightWidths(position,0);
			holder.item_right.setVisibility(View.GONE);
			holder.btn_index.setVisibility(View.GONE);
		}else if(position==1){
		...
		holder.btn_index.setText(String.valueOf(position));
		if (position!=0) {
			switch (position % 4) {
				case 1:
