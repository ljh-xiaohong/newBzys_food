    ...
    private void createBltService() {
        try {
            if (BltManager.getInstance().getmBluetoothAdapter() != null && BltManager.getInstance().getmBluetoothAdapter().isEnabled()) {
                bluetoothServerSocket = BltManager.getInstance().getmBluetoothAdapter().listenUsingRfcommWithServiceRecord("com.bapm.bzys.newBzys_food", BltContant.SPP_UUID);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void run(Handler handler) {
        ...
        while (true) {
            try {
                ...
                if (socket != null) {
                    UseDeviceSizeApplication.bluetoothSocket = socket;
                    //回调结果通知
