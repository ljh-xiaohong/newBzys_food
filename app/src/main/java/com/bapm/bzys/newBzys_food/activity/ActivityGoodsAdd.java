...
import com.bapm.bzys.newBzys_food.model.GoodsStatus;
...
//		setTheme(R.style.ActionSheetStyleIOS7);
	...
	public void initData(){
		...
		if(getIntent().hasExtra("goods")){
			...
			tv_title.setText("菜品编辑");
			btn_continue.setVisibility(View.GONE);
		}else{
			tv_title.setText("菜品新增");
		...
		switch (requstCode) {
			...
			case DadanUrl.GOODS_DETAIL_URL_CODE:{
				...
				if (ed_type.getText().toString().equals("必选商品")) {
					ed_unit.setText("位");
					ed_unit.setEnabled(false);
					layout_unit.setEnabled(false);
				}else{
					ed_unit.setText("");
					ed_unit.setEnabled(true);
					layout_unit.setEnabled(true);
				}
				break;
			}
		...
		switch (v.getId()) {
			case R.id.layout_goods_type:{
				...
				if (typeNames.size()>0) {
					...
					CustomToast.showToast(this, "请选择先到菜品大类去新增");
				}
				break;
			}
			...
			case R.id.btn_sure:{
				...
				try {
					...
					if (CommonUtil.isNull(tv_type_id.getText().toString())){
						loadDialog.dismiss();
						CustomToast.showToast(ActivityGoodsAdd.this,"请选择菜品大类");
...
//			imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
				imm.hideSoftInputFromWindow(ed_name.getWindowToken(), 0);
					showPicturePopupWindow(ActivityGoodsAdd.this);
...
//			imm.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults)
	{
		...
		if (requestCode == MY_PERMISSIONS_REQUEST_CALL_PHONE)
		{
			...
			{
				// Permission Denied
				CustomToast.showToast(ActivityGoodsAdd.this,getResources().getString(R.string.permission_tip));
				finish();
			}
		}
